...
태그 빌드는 `com.minseo.relaycode`, 워크플로에 고정된 현재 버전, 내부 테스터 전용 설정으로만
업로드됩니다.

## 실물 iPhone 검증 항목

- Gemma 4 E4B 약 3.66GB 또는 E2B 약 2.59GB 다운로드와 SHA-256 검증
- 다운로드 중 화면 잠금·앱 전환·강제 종료 후 기존 전송 재연결 및 진행률 복구
- 비행기 모드에서 내부 모델 추론과 연속 대화의 KV 접두사 재사용 수치
- 자동·저전력·균형·터보 모드의 첫 토큰 시간, 생성 속도, 메모리 경고, 10분 사용 시 발열
- Linux 부팅 후 `uname -a`, `free`, 파일 생성·수정·삭제
